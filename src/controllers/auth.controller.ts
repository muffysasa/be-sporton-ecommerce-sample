...
export const initiateAdmin = async (
    ...
    try {
        ...
        //Check is user data / entry is exist
        const count = await User.countDocuments({});
        if (count > 0) {
            res.status(400).json({
                message: "We can only have 1 admin user, if you want to create new admin user, please delet the user manually from the database"
            })
            return;
        }
